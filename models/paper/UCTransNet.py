...
class DownBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU'):
        ...
        self.nConvs = _make_nConv(in_channels, out_channels,
                                  nb_Conv, activation)
...
class UCTransNet(nn.Module):
    def __init__(self, config, in_channels=1, out_channels=2,
                 img_size=400, vis=False, **kwargs):
        super().__init__()
        self.vis = vis
        self.in_channels = in_channels
        self.out_channels = out_channels
        in_channels = config.base_channel
        self.inc = ConvBatchNorm(self.in_channels, in_channels)
        self.down1 = DownBlock(in_channels, in_channels*2, nb_Conv=2)
        self.down2 = DownBlock(in_channels*2, in_channels*4, nb_Conv=2)
        self.down3 = DownBlock(in_channels*4, in_channels*8, nb_Conv=2)
        self.down4 = DownBlock(in_channels*8, in_channels*8, nb_Conv=2)
        self.mtc = ChannelTransformer(
            config, vis, img_size, patchSize=config.patch_sizes,
            channel_num=[in_channels, in_channels*2,
        ...
        self.outc = nn.Conv2d(in_channels, out_channels,
