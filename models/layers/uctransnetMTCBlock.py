...
class ChannelTransformer(nn.Module):
    ...
    def forward(self, en1, en2, en3, en4):
        ...
        x1 = self.reconstruct_1(encoded1) \
            if en1 is not None else None
        x2 = self.reconstruct_2(encoded2) \
            if en2 is not None else None
        x3 = self.reconstruct_3(encoded3) \
            if en3 is not None else None
        x4 = self.reconstruct_4(encoded4) \
            if en4 is not None else None
...
class DefaultMTC(ChannelTransformer):
    def __init__(self, vis=False, img_size=400, base_channel=64,
                 patchSize=[16, 8, 4, 2]):
        channel_num = [base_channel * i for i in (1, 2, 4, 8)]
        config = {'KV_size': sum(channel_num), 'base_channel': base_channel,
                  'expand_ratio': 4, 'n_classes': 1,
                  'transformer': {
                      'attention_dropout_rate': 0.1, 'dropout_rate': 0,
                      'embeddings_dropout_rate': 0.1, 'num_heads': 4,
                      'num_layers': 4}}
        super().__init__(config, vis, img_size, channel_num, patchSize)
